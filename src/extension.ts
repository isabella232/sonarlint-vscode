...
export function activate(context: VSCode.ExtensionContext) {
  ...
  context.subscriptions.push(VSCode.commands.registerCommand(Commands.CONFIGURE_COMPILATION_DATABASE, configureCompilationDatabase));
  CompilationDatabaseAutomaticDetection()
...
async function ShowCDOptions(paths: VSCode.Uri[]) {
  ...
  if (paths.length == 1) {
    ...
    return VSCode.workspace.getConfiguration().update('sonarlint.PathToCompileCommands', paths[0].fsPath, VSCode.ConfigurationTarget.Workspace);
  }
  const items: IndexQP[] = [];
  ...
  if (selection) {
    ...
    await VSCode.workspace.getConfiguration().update('sonarlint.PathToCompileCommands', paths[selection.index].fsPath, VSCode.ConfigurationTarget.Workspace);
  }
}

async function CompilationDatabaseAutomaticDetection() {
  if (VSCode.workspace.getConfiguration().get('sonarlint.PathToCompileCommands')
      || !VSCode.workspace.getConfiguration().get('sonarlint.AskToSetCompileCommands')) {
    // Analysis already configured or option set tonot ask
    return;
  }
  ...
  const no: string = "No";
  const askMeLater: string = "Ask me later";
  return VSCode.window.showInformationMessage(
    "Compilation database detected. Would you like to activate C and C++ analysis?",
    yes, no, askMeLater).then(async (answer) => {
      switch (answer) {
        case "Yes":
          ShowCDOptions(paths);
          break;
        case "askMeLater":
          break;
        case "No":
          await VSCode.workspace.getConfiguration().update('sonarlint.AskToSetCompileCommands', false, VSCode.ConfigurationTarget.Workspace);
      }
    });
}
...
async function configureCompilationDatabase() {
  const paths = (await VSCode.workspace.findFiles(`**/compile_commands.json`))
    .filter((path) => FS.existsSync(path.fsPath));
  if (paths.length == 0) {
    VSCode.window.showWarningMessage(`No compilation database was found in the worksapace`);
  } else {
